const texts = {
  en: {
    title: "Sober Up",
    subtitle: "Worst-case scenario estimator",
    secPersonal: "1. Personal Info",
    gender: "Gender",
    male: "Male",
    female: "Female",
    weight: "Weight (kg)",
    ...
    ageLab: "Age",
    phOptional: "Optional",
    stomachLab: "Did you eat?",
    optEmpty: "No / Empty (Worst Case)",
    optAvg: "Average Meal",
    optFull: "Full / Greasy Meal",
    stopTime: "When did you stop?",
    timeHint: "Leave empty if 'Right Now'",
    secDrinks: "2. What did you drink?",
    selectLabel: "Select Drink Type:",
    optCustom: "-- Custom --",
    optBeerS: "Small Beer (330ml, 5%)",
    optBeerL: "Large Beer (500ml, 5%)",
    optWine: "Glass of Wine (100ml, 12%)",
    optWineL: "Large Wine (150ml, 12%)",
    optShot: "Shot / Spirit (40ml, 40%)",
    optCocktail: "Strong Cocktail (200ml, 10%)",
    btnAdd: "+ Add Drink",
    btnCalc: "CALCULATE SAFE TIME",
    resTitle: "Safe to Drive Estimate",
    resTime: "Time remaining:",
    h: "h",
    m: "m",
    discTitle: "⚠️ LEGAL DISCLAIMER",
    discText1:
      "Calculations use the Widmark/Watson formula with a conservative 0.10 ‰/h elimination rate to reach 0.00‰.",
    discText2:
  ...
  hu: {
    title: "Józanság",
    subtitle: "Legrosszabb esetre számolva",
    secPersonal: "1. Személyes Adatok",
    gender: "Nemed",
    male: "Férfi",
    female: "Nő",
    weight: "Testsúly (kg)",
    phWeight: "pl. 80",
    heightLab: "Magasság (cm)",
    ageLab: "Életkor",
    phOptional: "Opcionális",
    stomachLab: "Ettél valamit?",
    optEmpty: "Nem / Üres (Legrosszabb eset)",
    optAvg: "Átlagos étkezés",
    optFull: "Tele gyomor (Zsíros étel)",
    stopTime: "Mikor hagytad abba?",
    timeHint: "Hagyd üresen, ha 'Most'",
    secDrinks: "2. Mit ittál?",
    selectLabel: "Ital kiválasztása:",
    optCustom: "-- Egyéni --",
    optBeerS: "Kis Sör (330ml, 5%)",
    optBeerL: "Nagy Sör (500ml, 5%)",
    optWine: "Pohár Bor (100ml, 12%)",
    optWineL: "Nagy Bor (150ml, 12%)",
    optShot: "Feles / Rövid (40ml, 40%)",
    optCocktail: "Erős Koktél (200ml, 10%)",
    btnAdd: "+ Ital Hozzáadása",
    btnCalc: "MIKOR VEZETHETEK?",
    resTitle: "Legkorábban ekkor:",
    resTime: "Hátralévő idő:",
    h: "óra",
    m: "perc",
    discTitle: "⚠️ JOGI NYILATKOZAT",
    discText1:
      "A számítás a Widmark/Watson-képleten alapul, 0.10 ‰/óra (lassú) lebontással a 0.00‰ eléréséig.",
    discText2:
      "Ez az eszköz csak tájékoztató jellegű. NEM orvosi műszer és nem garancia.",
  ...
  document.querySelectorAll("[data-key]").forEach((el) => {
    ...
    if (texts[lang][k]) {
      if (el.tagName === "INPUT") {
        ...
        // Kezeli a select option szövegeket is
...
addDrinkBtn.addEventListener("click", () => {
  ...
  if (isNaN(vol) || isNaN(alc) || vol <= 0) {
    alert(
      lang === "hu"
        ? "Kérlek add meg a mennyiséget és százalékot!"
        : "Please enter volume and alcohol %!",
    );
    return;
  }
...
calculateBtn.addEventListener("click", () => {
  try {
    ...
    if (!weight || isNaN(weight)) {
      alert(
        lang === "hu" ? "Add meg a súlyodat!" : "Please enter your weight!",
      );
      return;
    }
    ...
    if (!isNaN(height) && !isNaN(age) && height > 0 && age > 0) {
      ...
      tbw = weight * genderRatio;
    }

    let peakBAC = (totalAlcoholGrams / tbw) * 0.8;

    let absorptionFactor = 1.0;
    if (stomach === "average") absorptionFactor = 0.85;
    if (stomach === "full") absorptionFactor = 0.7;

    peakBAC = peakBAC * absorptionFactor;

    const eliminationRate = 0.1;
    const hoursNeeded = peakBAC / eliminationRate;

    let absorptionDelay = 0.5;
    if (stomach === "full") absorptionDelay = 1.0;
    ...
    const safeDate = new Date(
      drinkTime.getTime() + (hoursNeeded + absorptionDelay) * 60 * 60 * 1000,
    );
    ...
    document.getElementById("totalAlcohol").innerText =
      totalAlcoholGrams.toFixed(1);
